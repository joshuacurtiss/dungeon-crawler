...
export default class Georgey extends Enemy {

    constructor(scene: Phaser.Scene, x: number, y: number) {
        super(scene, x, y, 'textures', 'georgey_idle_0')
        this.damageInflicted = 0.5
        this.speed = Phaser.Math.Between(50, 60)
        this.customOffset.set(3, 4)
        createStandardAnims(scene, 'georgey')
    ...
    setDirection(x:number, y:number) {
        ...
        if( this.direction.x || this.direction.y ) this.anims.play('georgey_run')
        else this.anims.play('georgey_idle')
    }
