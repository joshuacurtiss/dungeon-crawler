...
export default class Puggley extends Enemy {

    constructor(scene: Phaser.Scene, x: number, y: number) {
        super(scene, x, y, 'textures', 'puggley_idle_0')
        this.damageInflicted = 0.5
        this.speed = Phaser.Math.Between(50, 60)
        this.customOffset.set(3, 4)
        createStandardAnims(scene, 'puggley')
    ...
    setDirection(x:number, y:number) {
        ...
        if( this.direction.x || this.direction.y ) this.anims.play('puggley_run')
        else this.anims.play('puggley_idle')
    }
